...
// freeze once far enough out
	if (distance_to_player >= 384 || instance_exists(obj_pause)){
		image_speed = 0;
		exit;
	}
#endregion
...
#region damage check
// keep moving when hit, so placed here
if (hp <= 0 && round(iframes_timer) == 0){
	state = MOLBLIN_STATES.dead;
	instance_create_layer(x, y, LAY_FX, obj_item_dropping_death_explosion);
	instance_create_layer(x, y -16, LAY_FX, obj_item_dropping_death_explosion);
}
