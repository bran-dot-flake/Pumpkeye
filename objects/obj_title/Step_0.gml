...
index		+= sprite_get_speed(global.sprites.selector) / 60
...
if selected_level == 0 {
	...
	if y_in != 0 {
		...
		if selected_row == 1 and selected_column == 1 and window_get_fullscreen()
			selected_row += y_in;
...
if ip(input.Confirm){
	...
	if selected_level == 0 play_sfx(global.sounds.menu_confirm);
	selected_level++;
...
if ip(input.Deny){
	...
	if selected_level == 1 play_sfx(global.sounds.menu_back);
	selected_level--;
	target_x_offset = width;
	chck = true;
}
